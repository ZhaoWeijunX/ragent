...
@Component
public class IntentTop1Metric implements EvalMetric {

    @Override
    public List<MetricResult> compute(List<EvalScoreSample> samples) {
        MetricResult result = MetricSupport.sliceMean(
                "intent_top1",
                true,
                samples,
                s -> {
                    if (StrUtil.isBlank(s.getIntentL2())) {
                        return null;
                    }
                    return StrUtil.equals(s.getIntentPred(), s.getIntentL2()) ? 1.0 : 0.0;
                },
                null
        );
