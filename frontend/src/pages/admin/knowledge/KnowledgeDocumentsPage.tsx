...
import { BookOpen, Check, ChevronDown, ChevronRight, FileUp, FileImage, PlayCircle, RefreshCw, Trash2, Pencil, FileBarChart, X, Eye, MoreHorizontal, FileText, FileSpreadsheet, Link as LinkIcon, Download } from "lucide-react";
...
import type { KnowledgeBase, KnowledgeDocument, KnowledgeDocumentUploadPayload, KnowledgeDocumentChunkLog, PageResult, IngestionSpecSchema } from "@/services/knowledgeService";
import {
  deleteDocument,
  enableDocument,
  getKnowledgeBase,
...
import {
  budgetHintOf,
  buildIngestionSpec,
  DEFAULT_CONFIG_VALUES as FALLBACK_BUDGET,
  noChunkValueOf,
...
const uploadSchema = z
  .object({
    ...
    parseProfile: z.string().default("fast"),
    pipelineId: z.string().default(""),
    // 用户可控的全部自由度：块预算，字段与后端 schema 的 budgetFields 一一对应
    maxChars: z.string().default(FALLBACK_BUDGET.maxChars),
    overlapChars: z.string().default(FALLBACK_BUDGET.overlapChars),
    rowsPerChunk: z.string().default(FALLBACK_BUDGET.rowsPerChunk),
    toleranceFactor: z.string().default(FALLBACK_BUDGET.toleranceFactor)
  })
