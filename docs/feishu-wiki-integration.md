...
| 类 | 路径 | 职责 |
|----|------|------|
| `FeishuFetcher` | `ingestion/strategy/fetcher/` | 编排入口：鉴权、URL 分流、PDF/Markdown/plain 策略、组装 `FetchResult` |
...
├── FeishuDocxClientTest.java      # 含 PDF export_tasks 流程测试
├── FeishuWikiClientListNodesTest.java
└── FeishuFetcherTest.java         # 默认 PDF 路径 + markdown/plain 兼容
...
├── handler/RemoteFileFetcher.java
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `content-format` | `pdf` | `pdf` 走异步导出；`markdown` 走 `docs/v1/content`；`plain` 强制 `raw_content` |
| `fallback-to-plain-on-error` | `true` | 仅 `content-format=markdown` 时，Markdown API 失败回退 `raw_content` |
...
| 配置 | 行为 | 落盘 MIME | 解析器 |
|------|------|-----------|--------|
| `content-format: pdf`（**默认**） | 异步导出 PDF | `application/pdf`、`.pdf` | `MinerUDocumentParser` |
| `content-format: markdown` | Markdown API；失败可回退 plain | `text/markdown` / `text/plain` | `MarkdownDocumentParser` / `TikaDocumentParser` |
| `content-format: plain` | 始终 `raw_content` | `text/plain`、`.txt` | `TikaDocumentParser` |
| `fallback-to-plain-on-error: false` | 仅 markdown 模式；失败直接抛错 | — | — |
...
- 触发：`content-format=plain`，或 Markdown 失败且 `fallback-to-plain-on-error=true`
...
| 飞书 PDF 导出失败 | `ClientException` | 飞书 PDF 导出失败: ...；检查 `docs:document:export` 与文档阅读权限 |
| 飞书 PDF 导出超时 | `ClientException` | 飞书 PDF 导出超时，ticket=... |
| 飞书 Markdown 导出失败（无回退） | `ClientException` | 检查 `docs:document.content:read` |
| 飞书 Markdown 导出失败（已回退） | —（warn 日志） | 落盘为 `text/plain`，Parser 需允许 TEXT |
...
| `application.yaml` | `content-format: pdf`；配置 **`mineru.api-key`** |
...
| `FeishuDocxClientTest` | Markdown API、**PDF export_tasks 流程**、`raw_content`、导出失败状态码 |
| `FeishuFetcherTest` | **默认 PDF**、wiki→docx、markdown/plain 兼容、Markdown 失败回退 |
