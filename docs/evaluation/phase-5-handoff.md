...
**阶段 5 / M3 主路径基本对齐**：自建 + 可选 RAGAS、失败不回滚、独立 batch、异步进度/取消、Judge 端点由 Java 解析下发。完整 V1（人工覆盖、A/B、门禁、真实成本计量）属阶段 6+。
...
| 阶段 6：同版本 Run A/B（本轮） | 进行中；人工覆盖 / 门禁暂不实现 |
...
2. **若进阶段 6**：本轮仅做同版本 Run A/B 对比；人工覆盖 / 门禁 / CI 暂不实现
