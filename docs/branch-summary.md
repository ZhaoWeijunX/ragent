...
> 生成时间：2026-07-09（午后更新）  
> 基准分支：`main` / `origin/main`（`0ec5c90`）  
> 集成预览分支：`preview` / `origin/preview`（本地 `59cafe0`，含未提交增量）

本文档汇总 **origin 远程仓库** 各分支相对 `main` 的增量工作，便于了解进展、合并顺序与分支归档。

---

## 最近更新（2026-07-08 ～ 07-09）

### 知识库示例文档与意图树

在 `preview` 上新增/扩充三套 **多格式知识库** 及配套意图树，统一放在 `resources/docs/` 与 `docs/examples/intent-node-import/`：

| 知识库 | 目录 | 文档数 | 格式覆盖 | 意图节点 | 设计文档 |
|--------|------|--------|----------|----------|----------|
| **biz-security** 企业信息安全与合规 | `knowledge/biz/biz-security/` | 8 | md / txt / csv / xlsx / docx / pdf / png | 11（6 TOPIC） | `intent-tree-design.md` |
| **onboarding** 新员工入职与培训 | `knowledge/group/onboarding/` | 8 | 同上 | 11（6 TOPIC） | `intent-tree-design.md` |
| **ragent-test** 技术专栏（扩充） | `ragent-test/` | **52**（+18） | Markdown | **33**（+10，含 `rag-qa` 系列 9 TOPIC） | `intent-tree-design.md` |

**ragent-test 新增系列 `AI 知识问答篇/`（18 篇）**，覆盖 `StreamChatPipeline` 八阶段全链路：

- `rag-qa-pipeline`：问答全景（01）
- `rag-qa-memory`：记忆与摘要（02、03）
- `rag-qa-rewrite`：查询改写（04）
- `rag-qa-intent`：意图树 / 分类 Prompt / 歧义引导 / top_k（05～09）
- `rag-qa-retrieval`：多通道检索与后处理（10、11）
- `rag-qa-mcp`：MCP 链路（12、13）
- `rag-qa-prompt`：Prompt 组装（14）
- `rag-qa-stream`：流式正常/异常（15、16）
- `rag-qa-ratelimit`：对话排队限流（17、18）

**导入脚本**（均位于 `docs/examples/intent-node-import/`）：

| 脚本 | 用途 |
|------|------|
| `biz-security-intent-nodes-import.sql` | 信息安全知识库意图树 |
| `onboarding-intent-nodes-import.sql` | 入职培训知识库意图树 |
| `ragent-test-intent-nodes-import.sql` | 技术专栏意图树（已含 `rag-qa` 节点） |
| `sample-questions-import.sql` | 首页示例问题（ID 1～42，含 biz-security / onboarding / ragent-test 问答篇） |

导入后须执行：`redis-cli DEL ragent:intent:tree`。

### 代码与解析

| 提交 / 变更 | 内容 |
|-------------|------|
| `61d32db` | `DefaultIntentClassifier` 注释与结构整理 |
| `0f56932` | `MarkdownDocumentParser` 支持 `text/x-web-markdown` MIME |
| 工作区未提交 | `AI 知识问答篇` 18 篇文档、`intent-tree-design.md` 与 SQL / 示例问题增量 |

### 已推送提交（preview 线）

| 提交 | 说明 |
|------|------|
| `59cafe0` | onboarding 多格式知识库 + 示例问题 |
| `3323687` | biz-security 知识库 + ragent-test 文档整理 |
| `2e23b6d` | 合入飞书 PDF 默认导出分支 |
| `9368734` / `f042bb1` | 飞书 PDF 导出轮询与降级链 |
...
origin/main（干净基线，对齐 upstream/main）
 └── origin/preview（个人功能集成预览）
      ...
      ├── [2026-07-09] 示例知识库：biz-security / onboarding / ragent-test 问答篇 + 意图树 SQL
...
| `origin/preview` | 23+ | 5 | `59cafe0`（本地） | **个人功能集成预览** |
...
| 意图树 | 设计文档与导入 SQL（`main` 已 revert）；**扩充** ragent-test（52 篇 / 33 节点）、biz-security、onboarding 示例库 |
| 示例知识库 | `biz-security`（8 格式）、`onboarding`（8 格式）、`ragent-test/AI 知识问答篇`（18 篇） |
| 首页示例问题 | `sample-questions-import.sql`（42 条，覆盖 Demo / ragent-test / biz-security / onboarding） |
| 基础设施 | `infra-ai` 注释与结构整理（`main` 已 revert） |
| 文档 | `feishu-wiki-integration.md`、`docs/branch-summary.md`、`pdf-ingestion-example.md`、各知识库 `intent-tree-design.md` 等 |
...
**变更规模**：约 70+ 个文件，+5800 行量级（相对 `main`，含 PDF 改造与知识库示例文档增量）。

**同步状态**：领先 `main` 23+ 个提交，落后 5 个（`main` 上的 revert 提交及 `a593718` 文档版本，merge 时可能产生冲突）。本地 `preview` 在 `59cafe0` 之上尚有 **AI 知识问答篇** 与意图树 SQL 等待提交。
...
| 配置 | 飞书凭证、根目录 `.env` / 环境变量、基础设施 host 外置 |
...
4. **示例知识库灌入**：按各目录下 `intent-tree-design.md` 上传文档 → 分块 → 执行 `docs/examples/intent-node-import/*-intent-nodes-import.sql` → 清理 `ragent:intent:tree` 缓存；MinerU / Image 解析需对应服务与 VLM 配置。
5. **可归档分支**：`single_wiki`、`batch_wiki`、`markdown_support`、`mcp/weather` 在确认 `preview` 稳定后可删除。
6. **已归档**：`intent`、`add_model`、`my_preview`（已更名）无需进一步操作。
