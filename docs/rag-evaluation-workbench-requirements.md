...
7. **RAGAS 有成本与方差**：LLM Judge 耗时、耗费 Token，且存在 NaN、误判和单跑波动，需要任务化、重试、人工校准和成本控制。
...
| 产品 / 测试 | 查看总览、按业务场景验收、人工复核低分样本 | 查看与复核 |
| 普通用户 | 无评测工作台需求 | 无权限 |
...
| 评分 Score | 基于录制结果计算的确定性指标或 RAGAS 指标 |
| 基线 Baseline | 用于对比的历史运行 |
| Judge | 对回答与上下文进行语义评分的 LLM |
| 人工校准 Manual Override | 人工对 RAGAS 单样本评分进行覆盖，并保留审计 |
...
V1 支持对 RAGAS 单样本评分进行人工覆盖：

- 覆盖值范围为 0～1。
- 必须填写原因。
- 保存操作人和时间。
- 原始自动评分不可删除或覆盖。
- 聚合结果默认采用“人工评分优先、无人工评分时使用自动评分”。
- 报告必须显示人工覆盖数量及采用的聚合策略。
- 支持撤销人工覆盖，撤销动作同样记录审计。
...
| 方法 | 路径 | 说明 |
| --- | --- | --- |
| `POST` | `/records/{recordId}/overrides` | 新增人工评分 |
| `DELETE` | `/overrides/{overrideId}` | 撤销人工评分 |
| `GET` | `/runs/{runId}/overrides` | 查询人工覆盖 |
...
确定性指标不应写成一个大型条件分支。建议定义指标扩展接口，至少包含：
...
可将现有 `ragenteval` 的评分能力封装为独立服务。若评分服务不可用，Run 应完成确定性指标并标记 RAGAS 阶段失败，不能丢失录制结果。
...
5. 导出文件必须经过权限校验。
6. 所有人工覆盖、运行取消、数据集发布和 Judge 配置变更进入业务审计。
...
1. 可异步执行 RAGAS 五项指标，评分服务失败不影响确定性报告。
2. RAGAS NaN 或失败样本可单独重试，并展示最终状态。
3. 用户可选择两次同数据集版本的 Run 查看指标差异和样本回归。
4. 支持人工覆盖 RAGAS 单样本评分，并重新聚合 Overall 与 Intent L2 指标。
5. 自动评分、人工评分、操作人和原因均可追溯。
6. 阈值规则能给出 Run 的通过/失败结论，并列出触发规则。
7. 页面明确区分 ID-based Recall@K 与 RAGAS Context Recall。
...
1. 评估集、版本、样本、Run、录制、评分和人工复核的数据表及迁移脚本。
2. 后端数据集、Run、评分、报告、对比与复核 API。
