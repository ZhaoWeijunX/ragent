...
> 配套导入脚本：[resources/database/imports/intent-nodes/ragent-test-intent-nodes-import.sql](../../database/imports/intent-nodes/ragent-test-intent-nodes-import.sql)

本文档描述 `resources/knowledge-samples/ragent-test/` 目录下 **53 篇技术文档** 的意图树划分方案，用于 Ragent 意图分类与定向检索。
...
| 单库聚合 | 53 篇文档同属一个知识库，叶子节点共享 `kbId` 与 `collectionName` |
...
**节点统计**：1 DOMAIN + 6 CATEGORY + 27 TOPIC = **34 个节点**

系统交互节点（`sys` / `sys-welcome` / `sys-about-bot`）为全局共用，见 `resources/database/imports/intent-nodes/system-intent-nodes-import.sql`，不在本知识库脚本中重复导入。
...
| `tech-docs-patterns` | 设计模式全景 | `02` | 8 |
...
- `tech-docs-patterns`：Ragent 里用了哪些设计模式？
- `tech-docs-patterns`：策略模式在检索通道和文档解析里怎么落地？
- `tech-docs-patterns`：IngestionEngine 和 StreamChatPipeline 分别对应什么模式？
...
1. 创建 `ragent-test` 知识库并完成 **53 篇**文档灌入与分块。
...
3. 编辑 `resources/database/imports/intent-nodes/ragent-test-intent-nodes-import.sql`，全文替换：
