...
`filterCandidates\(\)` 做了什么？只保留 KB 类意图，且分数 \&gt;= 0\.35（`INTENT\_MIN\_SCORE`）：
...
`threshold` 默认 0\.8，`margin` 默认 0\.15。两者相减得到边界下限 0\.65。当次高分 / 最高分 \&lt; 0\.65 时，说明最高分品类有压倒性优势，直接走它就行，不需要引导。
...
- 归一化问题：`normalizeName\(\&\#34;OA系统的数据安全管理\&\#34;\)` → `\&\#34;oa系统的数据安全管理\&\#34;`

- 候选 DOMAIN 名称（去重后）：`\[\&\#34;OA系统\&\#34;, \&\#34;保险系统\&\#34;\]`

- 遍历 DOMAIN 名称：

    - `normalizeName\(\&\#34;OA系统\&\#34;\)` → `\&\#34;oa系统\&\#34;`，长度 4 \&gt;= 2

    - `\&\#34;oa系统的数据安全管理\&\#34;\.contains\(\&\#34;oa系统\&\#34;\)` → true
...
- 遍历 DOMAIN 名称：
    ...
    - `\&\#34;数据安全怎么管理\&\#34;\.contains\(\&\#34;oa系统\&\#34;\)` → false

    - `\&\#34;数据安全怎么管理\.contains\(\&\#34;保险系统\&\#34;\)` → false

- 全部 false，返回 false，不跳过，进入下一步三区间判定

电商单域名场景：用户问“3C 数码的退货政策”

- 候选 DOMAIN 名称（去重后）：`\[\&\#34;商品服务\&\#34;\]`（三个品类都属于同一个 DOMAIN）

- `”3c数码的退货政策”\.contains\(\&\#34;商品服务\&\#34;\)` → false
...
两个占位符：`\{topic\_name\}` 是触发歧义的主题名称，`\{options\}` 是品类列表。
...
和正常检索回复的事件流完全一样。没有专门的 guidance 事件类型，前端代码不需要做任何特殊适配——它只管接收 `message` 事件、拼接文本、展示给用户，不用关心这段文本是 LLM 生成的还是引导文案。
...
4. 即使分类器没拉开分差，`shouldSkipGuidance` 的字符串匹配也会发现改写后的问题里包含\&\#34;3C 数码\&\#34;，触发快速跳过
