...
跑完 RAGAS，某几条样本的 `faithfulness` 或 `context\_recall` 是 NaN。不是所有样本都 NaN，就偶发几条。其他指标可能正常，就某一两个指标出问题。

### **原因**

RAGAS 内部调 judge 拆 claims / statements 时，要求返回结构化的 JSON。但 RAGAS 的默认 prompt 是英文写的，judge 在处理中文内容时偶尔会出三种情况：

- 在 JSON 之外加一段中文解释——\&\#34;以下是我的分析：\{\.\.\.\}\&\#34;

- 用中文引号（\&\#34;\&\#34;）替代英文引号（\&\#34;\&\#34;），导致 JSON 解析失败

- 返回非标准格式——\&\#34;claims: \[\.\.\.\]\&\#34; 而不是纯 JSON 数组

RAGAS 的解析器碰到这些情况直接返回 NaN。

### **五层防御**

本项目在 `ragas\_judge\.py` 里做了五层 fallback，逐层兜底：
...
第一层用 `response\_format: json\_object` 强制 judge 输出合法 JSON，能解决大部分中文引号和格式问题。整批失败才退到第二层。第三层逐条跑的好处是：一条样本有问题不会拖垮整批——问题样本标 NaN，其他样本照常拿分。

第四层在 `compute\(\)` 里：全量跑完后，对仍然是 NaN 的样本单独重试一次，超时从默认 900s 放宽到 1200s——

```Python
# ragas_judge.py: NaN 样本逐条重试if failed:*
...
按 gpt\-5\.4\-mini 定价，一次完整评测大约 10~~20 元人民币（没有开多轮取均值）。听起来不多，但开发阶段一天跑 5 次就是 50~~100 元，一周下来几百元。
...
RAGAS 跑出来某条样本 `faithfulness` 只有 0\.5，你打开一看——回答完全忠实于 contexts，没有任何编造。或者反过来，某条样本 `answer\_correctness` 给了 0\.9，但人工一看回答明显遗漏了关键信息。
...
### **对策：人工抽查 \+** `per\_sample\.csv` **人工列校准**

RAGAS 分数不是最终裁决，而是一个自动化的初筛信号。关键发布决策不能只看自动分数，需要人工抽查低分样本并校准。

本项目在 `per\_sample\.csv` 里为每个 RAGAS 指标自动生成了一个 `\*\_manual` 空列，专门用于人工校准。

#### 2\.1 per\_sample\.csv 的结构

跑完 `python \-m eval rag report` 后，`eval/reports/v1\_xxx/per\_sample\.csv` 长这样：
...
用 Excel 或任何表格工具打开 `per\_sample\.csv`，按 `faithfulness` 或 `answer\_correctness` 升序排，找到分数最低的 3\~5 条。

**Step 2：人工判分**

打开对应样本的 `response`、`retrieved\_contexts`、`reference`（在 runs JSONL 文件里），人工判断 RAGAS 的打分对不对。

如果 RAGAS 给 `faithfulness` 打了 0\.5 但你判断应该是 0\.9（回答确实忠实于上下文），在 `faithfulness\_manual` 列填入 `0\.9`。
...
report 命令会自动检测 `per\_sample\.csv` 里的人工列。策略是人工优先，空值回退 RAGAS——

- `faithfulness\_manual` 有值 → 用人工分

- `faithfulness\_manual` 为空 → 用原始 RAGAS 分

然后基于校准后的 per\_sample 分数重新计算 overall 均值、`by\_intent\_l1` / `by\_intent\_l2` 分层均值，重新生成 `report\.md` 和 `slides\.html`。
...
> *大部分样本的自动评分是靠谱的，只有边界 case 需要人工介入。校准的结果会持久保留在 **`per\_sample\.csv`** 里——后续重跑 report 时自动生效，不需要重新审。*
> 
> 
...
> *第 6 项容易忽略。如果 runner 跑出来的 runs 文件里大量样本 **`retrieved\_contexts`** 为空（被 reject 或走了兜底），RAGAS 会把它们全部跳过。跑完发现可评 5 条，跳过 15 条——不是 RAGAS 的问题，是被评系统的检索链路有问题，回头查 Ragent。*
> 
> 
...
- **Judge 误判**：低分样本人工抽查，在 `per\_sample\.csv` 的 `\*\_manual` 列填入校准分，重跑 `python \-m eval rag report` 自动生效
