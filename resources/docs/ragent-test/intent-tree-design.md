...
### 3.5 CATEGORY：Ollama与本地部署（`local-llm`）
...
| SSE / 流式 | `infra-ai-chat-stream`（调用）或 `rag-eval-metrics-performance`（TTFT 评测） | — | 实现 vs 评测语境 |
...
3. 编辑 `docs/examples/ragent-test-intent-nodes-import.sql`，全文替换：
