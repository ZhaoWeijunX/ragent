-- 天气 MCP 意图节点与专属参数提取提示词
-- 前置：MCP Server 已注册 weather_query；执行后请刷新意图树缓存。
...
# 天气查询场景补充规则（weather_query）

## queryType
- 含「预报」「未来」「接下来」「明天」「后天」「这周」「一周」→ `forecast`
- 含「现在」「当前」「实时」「今天」「今日」且未强调未来 → `current`
- 仅问「XX天气怎么样」且未提未来 → 使用默认 `current`（可不输出 queryType）

## days（仅 queryType=forecast 时输出）
- 「明天」→ `2`；「后天」→ `3`
- 「一周」「7天」→ `7`；「三天」「3天」→ `3`
- 中文数字按阿拉伯数字输出；未提及天数可不输出（使用默认 3）
- 最大值 7，超过则输出 `7`

## city
- 提取城市名，去掉「的天气」等后缀；「北京市」可输出「北京」
- 不编造用户未提到的城市
...
**示例**：
{"city": "北京", "queryType": "forecast", "days": 3}$PROMPT$,
    update_time = NOW(),
    update_by = 'admin'
WHERE intent_code = 'weather-data'
  AND deleted = 0;
