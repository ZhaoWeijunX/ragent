-- You.com MCP 意图节点与专属参数提取提示词
-- 前置：mcp-server 已配置 youcom.search.api-key（或 YDC_API_KEY），并已注册 youcom_search。
-- 执行后请刷新意图树缓存。
...
# 联网搜索场景补充规则（youcom_search）

## query（必填）
- 提取用户真正要检索的核心关键词或问题，去掉口语前缀：「帮我搜一下」「查一下」「搜索」「联网查」等
- 保留时间、实体、主题等检索意图，例如：
  - 「今天有什么AI新闻？」→ `"今天 AI 新闻"`
  - 「OpenAI最近发布了什么」→ `"OpenAI 最新发布"`
- 不要把「联网」「搜索」等动作词当作 query 主体
- 用户问题本身就是完整检索意图时，可整体作为 query（适度去掉礼貌用语即可）

## count（非必填，默认 5，最大 20）
- 「一条/1条」→ `1`；「三条/3条」→ `3`；「前十/10条」→ `10`
- 未提及条数时不输出（使用默认 5）
- 超过 20 时输出 `20`

## freshness（非必填，enum: day / week / month / year）
- 「今天 / 今日 / 现在 / 当前 / 刚刚 / 实时 / 最新」→ day
- 「昨天 / 近两天 / 这两天」→ day
- 「最近 / 近期 / 新发布 / 新出 / 近几天」→ week（不要不传）
- 「这周 / 本周 / 近一周」→ week
- 「这个月 / 本月 / 近一个月」→ month
- 「今年 / 近一年」→ year
- 用户未强调时效范围时不输出（不限时间）
...
**示例**：
{"query": "2026年 AI 发展趋势", "count": 5, "freshness": "week"}$PROMPT$,
    update_time = NOW(),
    update_by = 'admin'
WHERE intent_code = 'youcom-search'
  AND deleted = 0;
