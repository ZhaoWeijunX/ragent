...
-- ============================================
-- RAG Evaluation Workbench Tables
-- 权威脚本：resources/database/evaluation/schema_eval_workbench.sql
-- ============================================
...
CREATE INDEX idx_eval_dataset_status ON t_eval_dataset (status) WHERE deleted = 0;
CREATE INDEX idx_eval_dataset_update_time ON t_eval_dataset (update_time DESC) WHERE deleted = 0;
...
CREATE UNIQUE INDEX uk_eval_dataset_version ON t_eval_dataset_version (dataset_id, version) WHERE deleted = 0;
CREATE INDEX idx_eval_dataset_version_dataset ON t_eval_dataset_version (dataset_id) WHERE deleted = 0;
CREATE INDEX idx_eval_dataset_version_status ON t_eval_dataset_version (status) WHERE deleted = 0;
...
CREATE UNIQUE INDEX uk_eval_case_query ON t_eval_case (dataset_version_id, query_id) WHERE deleted = 0;
CREATE INDEX idx_eval_case_version ON t_eval_case (dataset_version_id) WHERE deleted = 0;
CREATE INDEX idx_eval_case_intent_l2 ON t_eval_case (intent_l2) WHERE deleted = 0;
CREATE INDEX idx_eval_case_difficulty ON t_eval_case (difficulty) WHERE deleted = 0;
...
CREATE INDEX idx_eval_run_version ON t_eval_run (dataset_version_id) WHERE deleted = 0;
CREATE INDEX idx_eval_run_status ON t_eval_run (status) WHERE deleted = 0;
CREATE INDEX idx_eval_run_lease ON t_eval_run (lease_expire_at) WHERE deleted = 0 AND status NOT IN ('COMPLETED', 'PARTIAL_SUCCESS', 'FAILED', 'CANCELLED');
CREATE INDEX idx_eval_run_created ON t_eval_run (create_time DESC) WHERE deleted = 0;
...
CREATE UNIQUE INDEX uk_eval_record_run_case ON t_eval_record (run_id, case_id) WHERE deleted = 0;
CREATE INDEX idx_eval_record_run_status ON t_eval_record (run_id, status) WHERE deleted = 0;
CREATE INDEX idx_eval_record_trace ON t_eval_record (trace_id) WHERE deleted = 0 AND trace_id IS NOT NULL;
...
CREATE TABLE t_eval_score_batch (
    id                     VARCHAR(20)     NOT NULL PRIMARY KEY,
    run_id                 VARCHAR(20)     NOT NULL,
...
CREATE INDEX idx_eval_score_batch_run ON t_eval_score_batch (run_id, score_type) WHERE deleted = 0;
CREATE INDEX idx_eval_score_batch_status ON t_eval_score_batch (status) WHERE deleted = 0;
...
CREATE INDEX idx_eval_score_batch_metric ON t_eval_score (score_batch_id, metric_name) WHERE deleted = 0;
CREATE INDEX idx_eval_score_run_dim ON t_eval_score (run_id, dimension_type, dimension_value) WHERE deleted = 0;
CREATE INDEX idx_eval_score_record ON t_eval_score (record_id) WHERE deleted = 0 AND record_id IS NOT NULL;
...
CREATE TABLE t_eval_manual_override (
    id                VARCHAR(20)     NOT NULL PRIMARY KEY,
    run_id            VARCHAR(20)     NOT NULL,
...
CREATE UNIQUE INDEX uk_eval_manual_override_active ON t_eval_manual_override (run_id, record_id, metric_name) WHERE deleted = 0 AND status = 'ACTIVE';
CREATE INDEX idx_eval_manual_override_run ON t_eval_manual_override (run_id) WHERE deleted = 0;
